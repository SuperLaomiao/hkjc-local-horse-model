...
- [ ] Connect the validation-selected market-aware CatBoost WIN/PLACE champion to upcoming-race scoring in `SHADOW` mode.
  ...
  - Progress 2026-07-21: the frozen-lineage bridge now exists as `python/score_market_aware_candidate.py`, `src/probability-artifact.js`, and `shadow-score`; it hashes the CatBoost artifact, verifies report/feature-manifest agreement, rejects `observedAt >= postAt` and post-time generation, and normalizes every output to `SHADOW` / `PAPER_ONLY` / `RESEARCH_ONLY`. Remaining work is to surface those probabilities alongside the existing heuristic/no-market views for upcoming races.
