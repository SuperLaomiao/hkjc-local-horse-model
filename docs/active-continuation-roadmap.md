...
2. Verify the finite five-minute race-day LaunchAgent, its error log, due windows, backup freshness, and prospective coverage.
...
- [x] Verify the first newly published 2026/27 local race card end to end: official fixture identity, runner rows, SQLite `upcoming` state, and the next due T-window.
  - Completed 2026-09-09: synced the official 10-race 2026-09-06 Sha Tin results and 8-race 2026-09-09 Happy Valley card into private SQLite, captured R1 T-30/T-10/T-3 WIN/PLA/QIN/QPL before post time, and verified the repaired LaunchAgent completed automatic cycles with exit 0. The interval was tightened from ten to five minutes after the live canary showed that a ten-minute cadence can cross the entire 0–5 minute T-3 window.
- [ ] Wire the frozen scorer adapter and zero-stake decision builder into the installed `race-day-cycle` CLI; capture-only `score-not-configured` is not a prospective model lock.
...
- [x] Add prospective coverage, freshness, and backup-health reporting.
  ...
  - Completed 2026-07-22: `prospective-coverage` groups meeting/pool/window cells, separates missing racecard, offline, collector error, duplicate, not selling and missed-window reasons, reports snapshot/lock freshness plus latest checksum-backed backup, and never treats a missing lock as a miss. The gate declares its race/cell/lock/settlement/pool-window/backup minimums before touching ROI and publishes only aggregate dates, counts, ratios and exact deficits.
...
- [ ] Backfill timestamped historical SpeedPRO-style features and run same-cohort ablations.
  - Engineering ready 2026-07-22: per-runner source/time/identity provenance, train/validation/holdout/prospective coverage, and a frozen same-cohort ablation runner are implemented. The checkbox remains open because trustworthy historical pre-race backfill and a real fresh cohort are not yet available.
- [ ] Measure incremental lift from pool money, crowding, and odds movement after controlling for the market baseline.
  - Engineering ready 2026-07-22: the ablation runner compares base, SpeedPRO, pool-money, odds-movement, and combined policies on exactly the same complete races, selects on validation only, and reads prospective data once after freeze. No real lift is claimed while forward coverage is insufficient.
- [ ] Re-run QIN/QPL dependence and cold-odds experiments only when verified combination-book coverage exists; exact-order pools stay paper-only.
  - Engineering gate ready 2026-07-22: QIN/QPL evaluation now requires declared T-30/T-10/T-3 combination-book coverage and returns `BLOCKED_DATA` before ROI when coverage fails. The real experiment remains open.
- [ ] Sweep fixed stake, fractional Kelly, and exposure caps only for pools with positive prospective evidence; retain zero-stake `NO_BET` everywhere else.
  - Engineering gate ready 2026-07-22: fixed HK$10, fractional Kelly 0.1/0.25/0.5, and conservative-cap research curves exist. `BLOCKED_DATA`/`NO_GO` produces no sweep; all executable stakes remain zero even after manual research approval. A real sweep remains open until a pool earns positive prospective evidence.
...
| P6 | `race-day-cycle` | Phase A/D | implemented; this local deployment is enabled after explicit approval, but the current off-season cohort is still idle | Run one bounded due-window capture/score/lock cycle and keep all live evidence local. |
| P6 | `prospective-coverage-gate` | Phase D | implemented; real cohort remains BLOCKED_DATA | Declare coverage/backup minimums before ROI and expose exact aggregate deficits. |
| P7 | `prospective-promotion-state-machine` | Phase D | implemented; current real cohort remains BLOCKED_DATA and automation stops at REVIEW_REQUIRED | Compare frozen candidates on one fresh common cohort and enforce pool-specific evidence gates without enabling cash. |
| P8 | `post-gate-feature-staking-experiments` | Phase B/D | engineering implemented; real feature lift/QIN-QPL/staking results remain blocked on fresh data | Run same-cohort feature ablations and guarded zero-cash research sweeps only after data and promotion gates pass. |

## Latest continuation note

- 2026-09-09: Restored the 2026/27 local collection path after a season-opening audit. The private SQLite now contains 14,260 settled and 8 upcoming races after importing the official 2026-09-06 Sha Tin results and 2026-09-09 Happy Valley card. Before R1 post time, automatic/manual cycles captured R1 T-30/T-10/T-3 plus R2 T-30; every capture contains WIN/PLA/QIN/QPL odds and pool rows. The cycle truthfully reported `score-not-configured`, so no model lock was fabricated. The LaunchAgent exit-78 root cause was its Documents-based stdout/stderr boundary: the same command passed in a launchd-like minimal shell, while launchd could not update those logs. Reinstalling with logs under `~/Library/Logs/HKJC Local Horse Model/` produced repeated automatic cycles with exit 0 and an empty error log. The live canary then exposed that ten minutes can cross the entire T-3 capture band, so the lightweight collector now checks every five minutes without consuming Codex tokens. Added a TDD-backed `backup-db` command that uses SQLite online backup, SHA-256, atomic private manifest writes, and seven-backup managed retention; the first verified snapshot completed at 2026-09-09T10:36:24.671Z. The existing Codex automation is now daily at 08:00 HKT on `gpt-5.6-sol/high`, hard-capped at 60 minutes, with deterministic preflight, race-day/full versus non-race-day/light routing, compact reads, and P2 scorer wiring before P3 cohort accumulation/evaluation. Cash remains `NO_BET`. Exact next code task: add a failing integration test proving the CLI can load the frozen scorer and produce zero-stake immutable locks from a due captured window.

- 2026-08-13: Confirmed another off-season light-preflight run from official HKJC public sources before any deeper work. The official local results page still shows the latest local meeting at Happy Valley on 2026-07-15, while the official 2026/27 fixture/homepage continues to show the next local meeting on 2026-09-06 at Sha Tin, so today remains more than 14 days before the next local race and no local race card is yet inside the expected publication window. `hkjc:refresh -- --historyDays 1 --futureDays 60` probed only future September/October fixtures and reported every meeting from 2026-09-06 through 2026-10-11 as "race card not published or not parsable yet"; it fetched 0 settled races and 0 new race-card races. `sync-db` again left the approved private SQLite at 14,250 settled / 0 upcoming races; the worktree `data/upcoming/` cache still contains only stale June/July files, so there is still no verified 2026/27 card to sync. `race-day-cycle --dryRun` stayed healthy at 0 due / 0 captured / 0 scored / 0 locked with no next window, `prospective-settle` found 0 OPEN locks, `market-coverage` remains `partial-market-data` at 1,519/14,250 races with odds snapshots and 9/14,250 with pool snapshots, and `prospective-coverage --freezeDate 2026-07-22` still returns `BLOCKED_DATA` with 0/100 races, 0/400 usable cells, 0/200 locks, 0/150 settled locks, every required WIN/PLA/QIN/QPL T-30/T-10/T-3 cell still at 0/20, plus backup status `MISSING` with no checksum or age. This run also fixed one small operational bug with TDD: the documented `prospective-coverage --backupManifest ...` command no longer crashes on a missing manifest path and now reports backup `MISSING` fail-closed instead. LaunchAgent `com.superlaomiao.hkjc-race-day-cycle` still shows `StartInterval = 600`, `runs = 2974`, `last exit code = 78`, `state = not running`, and a plist with `Disabled = true`; its root stdout/stderr logs have not advanced since 2026-07-22 even though today's manual dry-run output is healthy, so keep treating that as a local scheduler observability/config checkpoint rather than a live-data failure. Focused tests: `node --test hkjc-horse-model/test/prospective-coverage.test.js`. Exact next command for the next daily preflight: `npm run hkjc:refresh -- --historyDays 1 --futureDays 60`.
- 2026-07-30: Confirmed the official off-season state before any deeper work. HKJC's public season-finale coverage says the 2025/26 local season ended at Happy Valley on 2026-07-15, and the official 2026/27 fixture release plus homepage now show the next local meeting on 2026-09-06 at Sha Tin. Because that next local meeting is still more than 14 days away and no local race card is yet published, this run stayed in light preflight mode. The mandatory local checks all matched that idle state after one real fix: `hkjc:refresh -- --historyDays 1 --futureDays 60` originally exposed a parser bug that mislabeled HKJC's unavailable `calmonth=08` fallback page as fake 2026-08-06/08-09/... meetings; a TDD slice added a regression test and changed fixture parsing to trust the returned `M/YYYY` page header, after which `refresh` only probed the official 2026-09-06/09-09/09-13/09-16/09-20/09-23/09-27 meetings and still found no published race cards. `sync-db` therefore kept the approved private SQLite at 14,250 settled / 0 upcoming races; `race-day-cycle --dryRun` reported 0 due windows; `prospective-settle` found 0 OPEN locks; and `prospective-coverage --freezeDate 2026-07-22` remained `BLOCKED_DATA` with 0/100 races, 0/400 usable cells, 0/200 locks, 0/150 settled locks, 0.00 settlement coverage, every required WIN/PLA/QIN/QPL T-30/T-10/T-3 cell still at 0/20, and backup status still `MISSING` with no checksum or age. LaunchAgent `com.superlaomiao.hkjc-race-day-cycle` remains installed on a 600-second interval, but `launchctl print` still shows the old `last exit code = 78` and the persisted root log files have not advanced since 2026-07-22 even though manual dry-run output is healthy, so treat that as an unresolved local scheduler observability/config checkpoint rather than a live-data failure. Focused tests: `node --test hkjc-horse-model/test/parser.test.js`. Exact next command for the next daily preflight: `npm run hkjc:refresh -- --historyDays 1 --futureDays 60`.
- 2026-07-23: Completed the unattended preflight on the fixed `codex/daily-hkjc` worktree and kept the system honestly idle. `refresh --historyDays 1 --futureDays 60` probed the published 2026-08-06/08-09/08-13/08-16/08-20/08-23/08-27/09-06/09-09/09-13/09-16/09-20 local meetings but no official race cards were yet published; `sync-db` therefore left SQLite at 14,250 settled / 0 upcoming races, `race-day-cycle --dryRun` reported 0 due windows, and `prospective-settle` found 0 OPEN locks. The same run then hardened the prospective coverage hot path in four TDD slices: freeze-aware SQLite loaders now prune historical races/snapshots/locks before Node materialization, freeze-date indexes were added for odds/pool snapshots plus prospective lock date filtering, and `BLOCKED_DATA` deficits now quantify absent settlement/pool-window coverage as `0` instead of `null`. Real `prospective-coverage --freezeDate 2026-07-22` now completes in about 6 seconds on the local production DB instead of hanging on a full-table load; `market-coverage` still reports only 1,519/14,250 races with odds snapshots and 9/14,250 with pool snapshots, so fresh cohort gating remains blocked and cash stays `NO_BET`. LaunchAgent remains installed on a 600-second interval; logs show repeated idle 0-due cycles, while `launchctl print` still reports a stale `last exit code = 78` that should be rechecked only after the first real upcoming racecard produces a non-idle cycle. Focused tests: `node --test hkjc-horse-model/test/sqlite-store.test.js hkjc-horse-model/test/prospective-coverage.test.js`. Exact next command when the first 2026/27 race card appears or a backup manifest is added: `npm run hkjc:prospective-coverage -- --db /Users/shi/Documents/赛马市场预测/hkjc-horse-model/data/hkjc.sqlite --freezeDate 2026-07-22 --backupManifest hkjc-horse-model/data/private/backup-manifest.json --output hkjc-horse-model/data/processed/prospective-coverage.json`.
- 2026-07-22: Completed P8 engineering gates without claiming data-dependent results. SpeedPRO rows now require source/time/identity provenance and report cohort-specific coverage; feature-policy ablations use complete identical races with validation-only selection and a frozen prospective read; QIN/QPL evaluation blocks before ROI without verified T-window combination books; staking research blocks for `BLOCKED_DATA`/`NO_GO` and can never create executable cash stake. Real P8 checkboxes stay open until trustworthy historical backfill and sufficient fresh forward evidence exist. Exact next task: collect the next race-day T-30/T-10/T-3 cohort, then run the frozen ablation report.
- 2026-07-22: Completed the P6 prospective coverage/freshness/backup-health implementation. `prospective-coverage-v1` consumes local races, market snapshots, collection events, immutable locks and an optional backup manifest; groups evidence by meeting, pool and T-window; and keeps absent locks under `MISSING_NOT_LOSS`. `prospective-data-gate-v1` freezes thresholds before ROI, defaults to 100 races / 400 usable cells / 200 locks / 150 settled locks / 75% settlement coverage / 20 usable cells per required pool-window plus a checksum-backed backup within 48 hours, and returns exact `BLOCKED_DATA` deficits with cash `NO_BET`. Public projection excludes race ids, lock ids, local paths, artifacts and SQLite labels. Full Node suite: 273/273; public privacy scan: 22 files, 0 violations. The gate itself remains unchecked until a genuinely fresh cohort reaches those declared minimums. Exact next task: P7/Task 5 identical-cohort prospective comparison tests.
