...
describe('research upgrade program', () => {
  ...
  it('orders research follow-up actions for the daily automation queue', () => {
    ...
    assert(program.followUpActions.some((item) => (
      ...
      && item.status === 'implemented'
      && item.automationExecutable === true
      && item.evidence.some((entry) => /prospective-coverage\.js/.test(entry))
    ...
    assert(program.followUpActions.some((item) => (
      item.id === 'post-gate-feature-staking-experiments'
      && item.status === 'implemented'
      && item.automationExecutable === true
      && item.evidence.some((entry) => /compare_feature_ablations\.py/.test(entry))
      && item.evidence.some((entry) => /combination-book|QIN\/QPL|coverage/i.test(entry))
      && item.evidence.some((entry) => /strategy-risk-report\.js|staking/i.test(entry))
      && item.remaining.some((entry) => /forward|prospective|真实/i.test(entry))
      && item.remaining.some((entry) => /NO_BET|cash/i.test(entry))
    )));
    assert(program.followUpActions.some((item) => (
